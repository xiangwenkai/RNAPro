...
from rnapro.model import sample_confidence
from rnapro.model.generator import (
    InferenceNoiseScheduler,
    TrainingNoiseSampler,
    sample_diffusion,
    sample_diffusion_training,
)
from rnapro.model.utils import simple_merge_dict_list
...
from .modules.pairformer import MSAModule, PairformerStack, RNATemplateEmbedder
from .modules.primitives import LinearNoBias
from .modules.ribonanzanet import RibonanzaNet, load_config_from_yaml, GatedSequenceFeatureInjector, GatedPairwiseFeatureInjector

logger = get_logger(__name__)


class RNAPro(nn.Module):
    """
    ...
    def __init__(self, configs) -> None:
        ...
        #     self.template_embedder = TemplateEmbedderAllatom(**configs.model.template_embedder)
        # elif configs.model.use_template == 'masked_templates':
        #     self.template_embedder = TemplateEmbedderCa(**configs.model.template_embedder)
        # else:
        ...
        if self.configs.model.use_RibonanzaNet2:
            config_path = os.path.join(self.configs.model.ribonanza_net_path, "pairwise.yaml")
            model_path = os.path.join(self.configs.model.ribonanza_net_path, "pytorch_model_fsdp.bin")
            rnet_config = load_config_from_yaml(config_path)
            self.ribonanza_net = RibonanzaNet(rnet_config)
            self.ribonanza_net.load_state_dict(torch.load(model_path), strict=True)
            ...
            s_input_dim = configs.model.input_embedder.c_token + 32 + 32 + 1
            ...
            self.projection_pairwise_features = nn.Sequential(
                                                            LinearNoBias(rnet_config.pairwise_dimension, self.c_z),
                                                            LayerNorm(self.c_z),
                                                            nn.ReLU(),
                                                            LinearNoBias(self.c_z, self.c_z),
                                                            LayerNorm(self.c_z),
                                                        )
            self.gated_sequence_feature_injector = GatedSequenceFeatureInjector(s_input_dim, s_input_dim)
            self.gated_pairwise_feature_injector = GatedPairwiseFeatureInjector(self.c_z, self.c_z)
            self.ribonanza_pairformer_stack = PairformerStack(n_blocks=4, 
                                                              n_heads=16, 
                                                              c_z=self.c_z, 
                                                              c_s=self.c_s)
